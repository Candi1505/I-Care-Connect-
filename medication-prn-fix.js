...
function showForCurrentMedication(){const panel=ensurePanel();if(!panel)return;const med=activeMedication();if(!isPrn(med)){panel.classList.add("hidden");currentMedication=null;return}if(currentMedication?.id!==med.id)resetPanel(panel);currentMedication=med;panel.classList.remove("hidden")}
function appendAssessment(){
 ...
 const forPain=q("#prn-for-pain",panel).value;if(!forPain)throw new Error("Choose whether this PRN is being given for pain");if(forPain==="No")return;
...
function install(){const form=q("#pin-form");if(!form||form.__prnPainInstalled)return;form.__prnPainInstalled=true;ensurePanel();form.addEventListener("submit",event=>{try{appendAssessment()}catch(error){event.preventDefault();event.stopImmediatePropagation();toast(error.message)}},{capture:true});document.addEventListener("click",event=>{if(event.target.closest("[data-administer],[data-mar-sign]"))setTimeout(showForCurrentMedication,40)})}

const CACHE="florence-shell-20260802-22";
const CORE_FIX="./core-ui-fixes-v2.js?v=20260802-2";
const SHELL=["./","./index.html","./styles.css?v=20260801-1","./config.js","./app.js?v=20260802-1","./operations.js?v=20260802-1","./staff-management.js?v=20260801-1","./setup-code-display.js?v=20260802-4","./live-refresh-controls.js?v=20260802-2","./notification-navigation.js?v=20260802-2",CORE_FIX,"./portal-participant-label.js?v=20260802-2","./portal-care-plan.js?v=20260802-2","./medication-prn-fix.js?v=20260802-1","./regular-medication-tab.js?v=20260802-1","./florence-readiness-controls.js?v=20260802-1","./remote-s8-verification.js?v=20260802-1","./sil.html","./sil.css?v=20260731-1","./sil.js?v=20260801-4","./manifest.webmanifest","./florence-icon.svg"];
self.addEventListener("install",event=>event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(SHELL)).then(()=>self.skipWaiting())));
self.addEventListener("activate",event=>event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(key=>key!==CACHE).map(key=>caches.delete(key)))).then(()=>self.clients.claim())));

async function withCoreFix(response){
...
self.addEventListener("fetch",event=>{
 ...
 event.respondWith((async()=>{
  try{
   ...
   const response=event.request.mode==="navigate"||network.headers.get("content-type")?.includes("text/html")?await withCoreFix(network):network;
   if(response.ok){const copy=response.clone();void caches.open(CACHE).then(cache=>cache.put(event.request,copy))}
   return response;
  }catch(_error){
   const hit=await caches.match(event.request);
   if(hit)return event.request.mode==="navigate"?withCoreFix(hit):hit;

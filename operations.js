...
document.addEventListener("click",async e=>{
 try{
  ...
  b=e.target.closest("[data-resolve-complaint]");if(b){const outcome=prompt("Record the complaint outcome");if(!outcome)return;const complaint=ops.complaints.find(item=>item.id===b.dataset.resolveComplaint);if(complaint?.portal_thread_id){const {error}=await B().db.rpc("reply_to_portal_complaint",{p_complaint_id:complaint.id,p_message:outcome,p_status:"Resolved"});if(error)throw error}else{const {error}=await B().db.from("complaints").update({status:"Resolved",outcome,actions_taken:outcome,resolved_at:new Date().toISOString()}).eq("id",b.dataset.resolveComplaint);if(error)throw error}await loadOperations();return B().toast(complaint?.portal_thread_id?"Outcome sent to the complainant":"Complaint resolved")}

-- Correct the timeline severity variable to use the timeline_severity enum.
-- The original text variable caused PostgreSQL to reject the client_timeline insert.
...
begin
  ...
  v_timeline_severity := (
    case p_severity when 'Severe' then 'High' when 'Moderate' then 'Moderate' else 'Low' end
  )::public.timeline_severity;
